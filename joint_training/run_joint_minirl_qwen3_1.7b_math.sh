...
#
# Based on: run_joint_grpo_qwen3_1.7b.sh
# Key changes:
#   - MiniRL loss (binary clip mask, REINFORCE-style gradient)
#   - Dr.GRPO advantage estimation (no std normalization)
#   - Token-level rollout IS correction (vLLM BF16 vs FSDP FP32)
#   - MATH dataset with MATH-500 + AIME-2025 validation
#   - seq-mean-token-sum aggregation (no per-token length normalization)
#
# Usage: bash recipe/joint_training/run_joint_minirl_qwen3_1.7b_math.sh
# ==============================================================================
...
if [ "$BASE_CKPT_MOUNT_SOURCE" = "$ROOT_MOUNT_SOURCE" ] && [[ "$BASE_CKPT_DIR" != /data-1/* ]]; then
    echo "[joint-minirl] WARNING: ${BASE_CKPT_DIR} resolves to the root filesystem (${BASE_CKPT_MOUNT_SOURCE}), not the large /data-1 volume." >&2
    echo "[joint-minirl] WARNING: On this host, prefer BASE_CKPT_DIR=/data-1/checkpoints." >&2
fi
...
if [ "${USE_REMOVE_PADDING}" = "True" ]; then
    if ! python3 -c "import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('flash_attn') else 1)" \
        >/dev/null 2>&1; then
        echo "[joint-minirl] WARNING: flash_attn is not installed; disabling USE_REMOVE_PADDING to avoid a late actor crash." >&2
        USE_REMOVE_PADDING=False
        if [ -z "${LOG_PROB_MICRO_BATCH_SIZE_WAS_SET}" ]; then
            LOG_PROB_MICRO_BATCH_SIZE=1
            echo "[joint-minirl] WARNING: LOG_PROB_MICRO_BATCH_SIZE was not set explicitly; reducing it to 1 for the dense-logits fallback path." >&2
        fi
    fi
fi
