#!/usr/bin/env bash
# Host-side KodCode Qwen3-1.7B CTX8K Stage1 queue from code format-cold-start weights.
set -euo pipefail
...
LOG_FILE=${LOG_FILE:-"${REPO_HOST}/recipe/on_policy_wdl_sft/code_task/run_code_task_kodcode_qwen3_1p7b_instruct_ctx8k_coldstart_stage1_queue.log"}
...
RUN_LABELS=("kodcode-qwen3-1p7b-coldstart-ctx8k-s1-beta0" "kodcode-qwen3-1p7b-coldstart-ctx8k-s1-beta01")
RUN_SCRIPTS=(
  "${REPO_CONTAINER}/recipe/on_policy_wdl_sft/code_task/run_s1_code_kodcode_qwen3_1p7b_instruct_ctx8k_coldstart_beta_0.sh"
  "${REPO_CONTAINER}/recipe/on_policy_wdl_sft/code_task/run_s1_code_kodcode_qwen3_1p7b_instruct_ctx8k_coldstart_beta_01.sh"
)
RUN_PREFIXES=(
...
if [ "${DRY_RUN:-0}" != "1" ] && [ "${ALLOW_KODCODE_QWEN3_1P7B_COLDSTART_CTX8K_STAGE1_TRAINING:-0}" != "1" ]; then
    echo "[kodcode qwen3 1p7b coldstart stage1 queue] ERROR: non-dry-run requires explicit ALLOW_KODCODE_QWEN3_1P7B_COLDSTART_CTX8K_STAGE1_TRAINING=1" >&2
    exit 1
fi
...
precheck_disk_or_stop() {
    ...
    [ -d "$INIT_MODEL_PATH" ] || { log "ERROR INIT_MODEL_PATH not found: $INIT_MODEL_PATH"; exit 1; }
    ...
    if [ "$free_gb" -lt "$MIN_FREE_GB" ]; then
        ...
        notify "KodCode Qwen3 1.7B coldstart Stage1 blocked" "Disk gate failed: free=${free_gb}G need>=${MIN_FREE_GB}G"
        exit 1
    fi
}
...
wait_for_tmux_run() {
    ...
    if [ -z "$ckpt" ] || [ "$step" -lt "$final_step" ] || [ -z "$metrics" ]; then
        ...
        notify "KodCode Qwen3 1.7B coldstart Stage1 failed" "${label} stopped before final checkpoint/metrics."
    ...
    notify "KodCode Qwen3 1.7B coldstart Stage1 complete" "${label} reached step ${step}."
}

launch_container() {
    local container_script="$1" tmux_name="$2"
    local host_script="${container_script/#$REPO_CONTAINER/$REPO_HOST}"
    local extra_env="INIT_MODEL_PATH='$INIT_MODEL_PATH' PROTECTED_CKPT_STEPS='$PROTECTED_CKPT_STEPS'"
    [ -f "$host_script" ] || { log "ERROR missing script: $host_script"; exit 1; }
    if [ "${DRY_RUN:-0}" = "1" ]; then
        log "DRY_RUN would launch ${tmux_name}: ${host_script} ${extra_env}"
...
log "kodcode qwen3 1p7b coldstart stage1 queue start START_INDEX=${START_INDEX} END_INDEX=${END_INDEX} DRY_RUN=${DRY_RUN:-0} INIT_MODEL_PATH=${INIT_MODEL_PATH} PROTECTED_CKPT_STEPS=${PROTECTED_CKPT_STEPS} MIN_FREE_GB=${MIN_FREE_GB} STATUS_FILE=${QUEUE_STATUS_FILE}"
notify "KodCode Qwen3 1.7B coldstart Stage1 queue started" "range=${START_INDEX}-${END_INDEX}; min_free_gb=${MIN_FREE_GB}; init=${INIT_MODEL_PATH}"
...
log "kodcode qwen3 1p7b coldstart stage1 queue complete"
notify "KodCode Qwen3 1.7B coldstart Stage1 queue complete" "status_file=${QUEUE_STATUS_FILE}"
