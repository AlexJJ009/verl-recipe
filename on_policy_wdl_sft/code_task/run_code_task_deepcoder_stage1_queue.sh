#!/usr/bin/env bash
# Host-side DeepCoder Stage1 queue with dense handoff retention.
set -euo pipefail
...
LOG_FILE=${LOG_FILE:-"${REPO_HOST}/recipe/on_policy_wdl_sft/code_task/run_code_task_deepcoder_stage1_queue.log"}
...
RUN_LABELS=("deepcoder-s1-beta0" "deepcoder-s1-beta01")
RUN_SCRIPTS=(
  "${REPO_CONTAINER}/recipe/on_policy_wdl_sft/code_task/run_s1_code_deepcoder_beta_0_retention.sh"
  "${REPO_CONTAINER}/recipe/on_policy_wdl_sft/code_task/run_s1_code_deepcoder_beta_01_retention.sh"
)
RUN_PREFIXES=(
...
if [ "${DRY_RUN:-0}" != "1" ] && [ "${ALLOW_DEEPCODER_STAGE1_TRAINING:-0}" != "1" ]; then
    echo "[deepcoder stage1 queue] ERROR: non-dry-run requires explicit ALLOW_DEEPCODER_STAGE1_TRAINING=1" >&2
    exit 1
fi
...
precheck_disk_or_stop() {
    ...
    if [ "$free_gb" -lt "$MIN_FREE_GB" ]; then
        ...
        notify "DeepCoder Stage1 blocked" "Disk gate failed: free=${free_gb}G need>=${MIN_FREE_GB}G"
        exit 1
    fi
}
...
wait_for_tmux_run() {
    ...
    if [ -z "$ckpt" ] || [ "$step" -lt "$final_step" ] || [ -z "$metrics" ]; then
        ...
        notify "DeepCoder Stage1 failed" "${label} stopped before final checkpoint/metrics."
    ...
    notify "DeepCoder Stage1 complete" "${label} reached step ${step}."
}
...
log "deepcoder stage1 queue start START_INDEX=${START_INDEX} END_INDEX=${END_INDEX} DRY_RUN=${DRY_RUN:-0} MIN_FREE_GB=${MIN_FREE_GB} STATUS_FILE=${QUEUE_STATUS_FILE}"
notify "DeepCoder Stage1 queue started" "range=${START_INDEX}-${END_INDEX}; min_free_gb=${MIN_FREE_GB}"
...
log "deepcoder stage1 queue complete"
notify "DeepCoder Stage1 queue complete" "status_file=${QUEUE_STATUS_FILE}"
