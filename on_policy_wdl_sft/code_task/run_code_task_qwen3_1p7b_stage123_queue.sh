...
[ -n "${STAGE123_STAGE12_PRODUCER_RECEIPT:-}" ] && STAGE123_STAGE12_PRODUCER_MODE=1
if [ "$DRY_RUN" != 1 ]; then
    ...
    if [ "$STAGE123_STAGE12_PRODUCER_MODE" = 1 ]; then
...
PY
        ...
        if [ "$STAGE123_STAGE12_PRODUCER_MODE" = 1 ]; then
            [ "$stage2_id" = "frac25-stage2" ] || continue
...
for row in "${stage2_rows[@]}"; do
    ...
    if [ "$STAGE123_STAGE12_PRODUCER_MODE" = 1 ] && [ "$stage2_id" != "frac25-stage2" ]; then
        continue
    fi
...
PY
    ...
    stage2_ckpt=$(launch_and_wait "$stage2_tmux" "$stage2_prefix" "$STAGE2_STEPS" \
        ...
        STAGE123_STAGE12_PRODUCER_RECEIPT="${STAGE123_STAGE12_PRODUCER_RECEIPT:-}" STAGE123_STAGE12_CALIBRATION_REPORT="${STAGE123_STAGE12_CALIBRATION_REPORT:-}" STAGE123_STAGE12_CALIBRATION_POLICY="${STAGE123_STAGE12_CALIBRATION_POLICY:-}" STAGE123_STAGE12_CALIBRATION_HISTORY_INDEX="${STAGE123_STAGE12_CALIBRATION_HISTORY_INDEX:-}" STAGE123_STAGE12_CALIBRATION_PREDICTION_CONTRACT="${STAGE123_STAGE12_CALIBRATION_PREDICTION_CONTRACT:-}" STAGE123_STAGE12_RECEIPT_MAX_AGE_SECONDS="${STAGE123_STAGE12_RECEIPT_MAX_AGE_SECONDS:-86400}" STAGE123_STAGE12_RECEIPT_FUTURE_SKEW_SECONDS="${STAGE123_STAGE12_RECEIPT_FUTURE_SKEW_SECONDS:-300}")
    ...
    if [ "$STAGE123_STAGE12_PRODUCER_MODE" = 1 ]; then
        record "$chain" stage2 producer_completed "model2=$stage2_model2 provenance=$stage3_provenance"
        echo "[STAGE123 QUEUE] Stage2 producer complete; Stage3 remains pending regenerated preflight"
        exit 0
    fi
