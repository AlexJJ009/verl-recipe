#!/usr/bin/env bash
# Host-side KodCode Instruct-2507 CTX8K P40 matched-beta Stage2 queue.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export LOG_FILE=${LOG_FILE:-/data-1/verl07/verl/recipe/on_policy_wdl_sft/code_task/run_code_task_kodcode_instruct2507_ctx8k_stage2_p40_queue.log}
export QUEUE_TMUX=${QUEUE_TMUX:-code_task_kodcode_i2507_ctx8k_s2_p40_queue}
export QUEUE_MODE=kodcode_instruct2507_ctx8k_stage2_p40
export START_INDEX=${START_INDEX:-0}
export END_INDEX=${END_INDEX:-1}
export QUEUE_CONTINUE_ON_FAILURE=${QUEUE_CONTINUE_ON_FAILURE:-0}
export QUEUE_STATUS_FILE=${QUEUE_STATUS_FILE:-/data-1/verl07/verl/recipe/on_policy_wdl_sft/code_task/run_code_task_kodcode_instruct2507_ctx8k_stage2_p40_queue_status.tsv}
export MIN_FREE_GB=${MIN_FREE_GB:-300}
export MODEL2_ROOT=${MODEL2_ROOT:-/data-1/model_weights/code_task/kodcode_instruct2507_ctx8k_stage2_p40}
...
export STAGE2_BETA0_HANDOFF_STEP=${STAGE2_BETA0_HANDOFF_STEP:-40}
export STAGE2_BETA01_HANDOFF_STEP=${STAGE2_BETA01_HANDOFF_STEP:-40}
...
export STAGE2_BETA0_TRAIN_FILE=${STAGE2_BETA0_TRAIN_FILE:-/data-1/dataset/code/verl_rl/kodcode_stage2_after_s1_seed20260604_beta0_p40_handoff.parquet}
export STAGE2_BETA01_TRAIN_FILE=${STAGE2_BETA01_TRAIN_FILE:-/data-1/dataset/code/verl_rl/kodcode_stage2_after_s1_seed20260604_beta01_p40_handoff.parquet}
...
if [ "${DRY_RUN:-0}" != "1" ] && [ "${ALLOW_KODCODE_INSTRUCT2507_CTX8K_STAGE2_P40_TRAINING:-0}" != "1" ]; then
    echo "[kodcode instruct2507 ctx8k stage2 p40 queue] ERROR: non-dry-run requires explicit ALLOW_KODCODE_INSTRUCT2507_CTX8K_STAGE2_P40_TRAINING=1" >&2
    exit 1
fi
if [ "${ALLOW_KODCODE_INSTRUCT2507_CTX8K_STAGE2_P40_TRAINING:-0}" = "1" ]; then
...
create_or_verify_shard() {
    ...
    if [ -f "$output" ] && [ -f "$(python3 -c 'from pathlib import Path; import sys; print(Path(sys.argv[1]).with_suffix(".manifest.json"))' "$output")" ]; then
        echo "[kodcode instruct2507 ctx8k stage2 p40 queue] verifying ${label} shard: ${output}"
        run_shard_script "$output" "${args[@]}" --verify-only
    else
        echo "[kodcode instruct2507 ctx8k stage2 p40 queue] creating ${label} shard: ${output}"
        run_shard_script "$output" "${args[@]}"
    fi
}

if [ "${CREATE_STAGE2_SHARDS:-1}" = "1" ]; then
    create_or_verify_shard "beta0-p40" "$STAGE2_BETA0_HANDOFF_STEP" "$STAGE2_BETA0_TRAIN_FILE"
    create_or_verify_shard "beta01-p40" "$STAGE2_BETA01_HANDOFF_STEP" "$STAGE2_BETA01_TRAIN_FILE"
fi
