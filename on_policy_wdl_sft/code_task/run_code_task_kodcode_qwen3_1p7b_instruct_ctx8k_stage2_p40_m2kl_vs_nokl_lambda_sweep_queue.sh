#!/usr/bin/env bash
# Formal Qwen3-1.7B KodCode CTX8K P40 Stage2 no-KL vs model2-only KL lambda sweep.
# Matrix: beta {0.0,0.1} x fusion_lambda {0.5..0.9} x KL {off, model2-only}.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export LOG_FILE=${LOG_FILE:-/data-1/verl07/verl/recipe/on_policy_wdl_sft/code_task/run_code_task_kodcode_qwen3_1p7b_instruct_ctx8k_stage2_p40_m2kl_vs_nokl_lambda_sweep_queue.log}
...
export QUEUE_STATUS_FILE=${QUEUE_STATUS_FILE:-/data-1/verl07/verl/recipe/on_policy_wdl_sft/code_task/run_code_task_kodcode_qwen3_1p7b_instruct_ctx8k_stage2_p40_m2kl_vs_nokl_lambda_sweep_status.tsv}
...
export STAGE2_BETA0_STAGE1_PREFIX=${STAGE2_BETA0_STAGE1_PREFIX:-ONPOLICY-SFT-Qwen3-1P7B-INSTRUCT-CODE-KODCODE-CTX8K-S1-BETA0-V1}
export STAGE2_BETA01_STAGE1_PREFIX=${STAGE2_BETA01_STAGE1_PREFIX:-ONPOLICY-SFT-Qwen3-1P7B-INSTRUCT-CODE-KODCODE-CTX8K-S1-BETA01-V1}
export STAGE2_BETA0_TRAIN_FILE=${STAGE2_BETA0_TRAIN_FILE:-/data-1/dataset/code/verl_rl/kodcode_stage2_after_s1_seed20260604_qwen3_1p7b_beta0_p40_handoff_s2steps60.parquet}
export STAGE2_BETA01_TRAIN_FILE=${STAGE2_BETA01_TRAIN_FILE:-/data-1/dataset/code/verl_rl/kodcode_stage2_after_s1_seed20260604_qwen3_1p7b_beta01_p40_handoff_s2steps60.parquet}
export START_INDEX=${START_INDEX:-0}
export END_INDEX=${END_INDEX:-19}
...
export MODEL2_ROOT=${MODEL2_ROOT:-/data-1/model_weights/code_task/kodcode_qwen3_1p7b_instruct_ctx8k_stage2_p40}
...
if [ "${DRY_RUN:-0}" != "1" ] && [ "${ALLOW_KODCODE_QWEN3_1P7B_INSTRUCT_CTX8K_STAGE2_P40_M2KL_VS_NOKL_TRAINING:-0}" != "1" ]; then
    echo "[kodcode qwen3 1p7b stage2 p40 m2kl vs nokl queue] ERROR: non-dry-run requires explicit ALLOW_KODCODE_QWEN3_1P7B_INSTRUCT_CTX8K_STAGE2_P40_M2KL_VS_NOKL_TRAINING=1" >&2
    exit 1
fi
if [ "${ALLOW_KODCODE_QWEN3_1P7B_INSTRUCT_CTX8K_STAGE2_P40_M2KL_VS_NOKL_TRAINING:-0}" = "1" ]; then
...
create_or_verify_shard() {
    ...
    if [ -f "$output" ] && [ -f "$(python3 -c 'from pathlib import Path; import sys; print(Path(sys.argv[1]).with_suffix(".manifest.json"))' "$output")" ]; then
        echo "[kodcode qwen3 1p7b stage2 p40 m2kl vs nokl queue] verifying ${label} shard: ${output}"
        run_shard_script "$output" "${args[@]}" --verify-only
    else
        echo "[kodcode qwen3 1p7b stage2 p40 m2kl vs nokl queue] creating ${label} shard: ${output}"
        run_shard_script "$output" "${args[@]}"
    fi
}

if [ "${CREATE_STAGE2_SHARDS:-1}" = "1" ]; then
    create_or_verify_shard "qwen3-1p7b-beta0-p40" "$STAGE2_BETA0_P40_HANDOFF_STEP" "$STAGE2_BETA0_TRAIN_FILE"
    create_or_verify_shard "qwen3-1p7b-beta01-p40" "$STAGE2_BETA01_P40_HANDOFF_STEP" "$STAGE2_BETA01_TRAIN_FILE"
fi
