#!/usr/bin/env bash
# KodCode Instruct-2507 CTX8K P40 Stage2 submodel-KL smoke wrapper.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

if [ "${DRY_RUN:-0}" != "1" ] && [ "${ALLOW_SUBMODEL_KL_SMOKE:-0}" != "1" ]; then
    echo "[submodel-kl smoke] ERROR: non-dry-run requires ALLOW_SUBMODEL_KL_SMOKE=1" >&2
    exit 1
fi

export RUN_PREFIX=${RUN_PREFIX:-CODE-S2-KODCODE-INSTRUCT2507-CTX8K-P40-BETA01-SUBKL-SMOKE}
...
export CODE_TRAIN_FILE=${CODE_TRAIN_FILE:-/data-1/dataset/code/verl_rl/kodcode_stage2_after_s1_seed20260604_beta01_p40_handoff.parquet}
export TRAIN_FILE=${TRAIN_FILE:-$CODE_TRAIN_FILE}
export MODEL2_LABEL=${MODEL2_LABEL:-kodcode_i2507_ctx8k_beta01_p40_subkl_smoke}
export MODEL2_CACHE_TAG=${MODEL2_CACHE_TAG:-kod_i2507_p40_b01_subkl_smoke}
export STAGE1_MERGED_MODEL_ROOT=${STAGE1_MERGED_MODEL_ROOT:-/data-1/model_weights/code_task/kodcode_instruct2507_ctx8k_stage2_p40/beta01}
...
export GENERATION_MICRO_BATCH_SIZE=${GENERATION_MICRO_BATCH_SIZE:-4}
...
export CHECKPOINT_SAVE_CONTENTS=${CHECKPOINT_SAVE_CONTENTS:-"[model,extra]"}
