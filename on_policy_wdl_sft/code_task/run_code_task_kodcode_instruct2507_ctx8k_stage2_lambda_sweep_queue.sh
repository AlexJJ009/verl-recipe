#!/usr/bin/env bash
# Host-side KodCode Instruct-2507 CTX8K Stage2 fusion_lambda sweep queue.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export LOG_FILE=${LOG_FILE:-/data-1/verl07/verl/recipe/on_policy_wdl_sft/code_task/run_code_task_kodcode_instruct2507_ctx8k_stage2_lambda_sweep_queue.log}
...
export QUEUE_STATUS_FILE=${QUEUE_STATUS_FILE:-/data-1/verl07/verl/recipe/on_policy_wdl_sft/code_task/run_code_task_kodcode_instruct2507_ctx8k_stage2_lambda_sweep_queue_status.tsv}
...
export STAGE2_BETA01_P60_HANDOFF_STEP=${STAGE2_BETA01_P60_HANDOFF_STEP:-60}
...
export STAGE2_BETA01_P60_TRAIN_FILE=${STAGE2_BETA01_P60_TRAIN_FILE:-/data-1/dataset/code/verl_rl/kodcode_stage2_after_s1_seed20260604_beta01_p60_handoff.parquet}
...
if [ "${DRY_RUN:-0}" != "1" ] && [ "${ALLOW_KODCODE_INSTRUCT2507_CTX8K_STAGE2_LAMBDA_SWEEP_TRAINING:-0}" != "1" ]; then
    echo "[kodcode instruct2507 ctx8k stage2 lambda sweep queue] ERROR: non-dry-run requires explicit ALLOW_KODCODE_INSTRUCT2507_CTX8K_STAGE2_LAMBDA_SWEEP_TRAINING=1" >&2
    exit 1
fi
if [ "${ALLOW_KODCODE_INSTRUCT2507_CTX8K_STAGE2_LAMBDA_SWEEP_TRAINING:-0}" = "1" ]; then
...
create_or_verify_shard() {
    ...
    if [ -f "$output" ] && [ -f "$(python3 -c 'from pathlib import Path; import sys; print(Path(sys.argv[1]).with_suffix(".manifest.json"))' "$output")" ]; then
        echo "[kodcode instruct2507 ctx8k stage2 lambda sweep queue] verifying ${label} shard: ${output}"
        run_shard_script "$output" "${args[@]}" --verify-only
    else
        echo "[kodcode instruct2507 ctx8k stage2 lambda sweep queue] creating ${label} shard: ${output}"
        run_shard_script "$output" "${args[@]}"
    fi
}

if [ "${CREATE_STAGE2_SHARDS:-1}" = "1" ]; then
    create_or_verify_shard "beta01-p40" "$STAGE2_BETA01_P40_HANDOFF_STEP" "$STAGE2_BETA01_P40_TRAIN_FILE"
    if [ "${CREATE_P60_STAGE2_SHARD:-0}" = "1" ]; then
        create_or_verify_shard "beta01-p60" "$STAGE2_BETA01_P60_HANDOFF_STEP" "$STAGE2_BETA01_P60_TRAIN_FILE"
    fi
fi
