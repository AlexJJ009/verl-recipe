#!/usr/bin/env bash
# Host-side DeepCoder Instruct-2507 response-8K Stage1 queue.
set -euo pipefail
...
LOG_FILE=${LOG_FILE:-"${REPO_HOST}/recipe/on_policy_wdl_sft/code_task/run_code_task_deepcoder_instruct2507_r8k_stage1_queue.log"}
...
RUN_LABELS=("deepcoder-instruct2507-r8k-s1-beta0" "deepcoder-instruct2507-r8k-s1-beta01")
RUN_SCRIPTS=(
  "${REPO_CONTAINER}/recipe/on_policy_wdl_sft/code_task/run_s1_code_deepcoder_instruct2507_r8k_beta_0.sh"
  "${REPO_CONTAINER}/recipe/on_policy_wdl_sft/code_task/run_s1_code_deepcoder_instruct2507_r8k_beta_01.sh"
)
RUN_PREFIXES=(
...
if [ "${DRY_RUN:-0}" != "1" ] && [ "${ALLOW_DEEPCODER_INSTRUCT2507_R8K_STAGE1_TRAINING:-0}" != "1" ]; then
    echo "[deepcoder instruct2507 r8k stage1 queue] ERROR: non-dry-run requires explicit ALLOW_DEEPCODER_INSTRUCT2507_R8K_STAGE1_TRAINING=1" >&2
    exit 1
fi
...
precheck_disk_or_stop() {
    ...
    if [ "$free_gb" -lt "$MIN_FREE_GB" ]; then
        ...
        notify "DeepCoder Instruct2507 R8K Stage1 blocked" "Disk gate failed: free=${free_gb}G need>=${MIN_FREE_GB}G"
        exit 1
    fi
}
...
wait_for_tmux_run() {
    ...
    if [ -z "$ckpt" ] || [ "$step" -lt "$final_step" ] || [ -z "$metrics" ]; then
        ...
        notify "DeepCoder Instruct2507 R8K Stage1 failed" "${label} stopped before final checkpoint/metrics."
    ...
    notify "DeepCoder Instruct2507 R8K Stage1 complete" "${label} reached step ${step}."
}
...
log "deepcoder instruct2507 r8k stage1 queue start START_INDEX=${START_INDEX} END_INDEX=${END_INDEX} DRY_RUN=${DRY_RUN:-0} MIN_FREE_GB=${MIN_FREE_GB} STATUS_FILE=${QUEUE_STATUS_FILE}"
notify "DeepCoder Instruct2507 R8K Stage1 queue started" "range=${START_INDEX}-${END_INDEX}; min_free_gb=${MIN_FREE_GB}"
...
log "deepcoder instruct2507 r8k stage1 queue complete"
notify "DeepCoder Instruct2507 R8K Stage1 queue complete" "status_file=${QUEUE_STATUS_FILE}"
