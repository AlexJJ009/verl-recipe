#!/usr/bin/env bash
# Monitor for Qwen3-1.7B KodCode cold-start CTX8K P40 Stage2 no-KL vs model2-only KL queue.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export QUEUE_MODE=kodcode_qwen3_1p7b_coldstart_ctx8k_stage2_p40_m2kl_vs_nokl
export MONITOR_NAME=${MONITOR_NAME:-code_task_kodcode_qwen3_1p7b_coldstart_ctx8k_stage2_p40_m2kl_vs_nokl}
