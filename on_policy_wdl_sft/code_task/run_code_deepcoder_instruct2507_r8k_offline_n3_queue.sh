#!/usr/bin/env bash
# Host-side full official code offline eval queue for DeepCoder Instruct-2507 R8K Stage1 checkpoints.
set -euo pipefail
...
BIGCODEBENCH_OFFICIAL_EVAL_PARALLEL=${BIGCODEBENCH_OFFICIAL_EVAL_PARALLEL:-$CODE_OFFICIAL_EVAL_PARALLEL}
...
ACTOR_DIRS=(
    "/data-1/checkpoints/ONPOLICY-SFT-Qwen3-4B-INSTRUCT2507-CODE-DEEPCODER-R8K-S1-BETA0-V1_1782133285/global_step_120/actor"
    "/data-1/checkpoints/ONPOLICY-SFT-Qwen3-4B-INSTRUCT2507-CODE-DEEPCODER-R8K-S1-BETA0-V1_1782133285/global_step_150/actor"
    "/data-1/checkpoints/ONPOLICY-SFT-Qwen3-4B-INSTRUCT2507-CODE-DEEPCODER-R8K-S1-BETA01-V1_1782232869/global_step_115/actor"
    "/data-1/checkpoints/ONPOLICY-SFT-Qwen3-4B-INSTRUCT2507-CODE-DEEPCODER-R8K-S1-BETA01-V1_1782232869/global_step_150/actor"
)
MERGED_DIRS=(
...
validation_parquet_for_benchmark() {
    local benchmark="$1"
    case "${benchmark}" in
        humaneval) echo "/data-1/dataset/code/verl_rl/online_full_humaneval_plus/official_humaneval_plus_val.parquet" ;;
        mbpp) echo "/data-1/dataset/code/verl_rl/online_full_mbpp_plus/official_mbpp_plus_val.parquet" ;;
        bigcodebench) echo "/data-1/dataset/code/verl_rl/online_full_bigcodebench/official_bigcodebench_val.parquet" ;;
        livecodebench)
            ...
            ;;
        *) echo "ERROR: unsupported benchmark ${benchmark}" >&2; return 2 ;;
    esac
}

official_parallel_for_benchmark() {
    local benchmark="$1"
    case "${benchmark}" in
        bigcodebench) echo "${BIGCODEBENCH_OFFICIAL_EVAL_PARALLEL}" ;;
        *) echo "${CODE_OFFICIAL_EVAL_PARALLEL}" ;;
    esac
}
...
validate_inputs() {
    ...
    if [[ " ${BENCHMARKS} " == *" livecodebench "* ]]; then
...
PY' \
    ...
    for i in "${!LABELS[@]}"; do
        if merged_ready "${MERGED_DIRS[$i]}"; then
            ...
            record_status "$i" "${LABELS[$i]}" "all" "merge-check" "present" "${MERGED_DIRS[$i]}"
        else
            log "merged missing; queue will merge before first benchmark: ${LABELS[$i]} -> ${MERGED_DIRS[$i]}"
            record_status "$i" "${LABELS[$i]}" "all" "merge-check" "missing" "${MERGED_DIRS[$i]}"
        fi
    done
    if [ "${ok}" != "1" ]; then
        exit 2
...
run_case() {
    ...
    local official_parallel
    official_parallel="$(official_parallel_for_benchmark "${benchmark}")"

    if [[ "${docker_gpus_arg}" != "all" && "${docker_gpus_arg}" != \"* ]]; then
        docker_gpus_arg="\"${docker_gpus_arg}\""
    fi

    log "case: index=${i} label=${label} benchmark=${benchmark} actor=${actor} merged=${merged} validation=${parquet} output=${case_dir} official_parallel=${official_parallel}"
...
main() {
    ...
    log "DeepCoder Instruct2507 R8K unified N=3 offline eval queue start"
    log "settings: labels=${#LABELS[@]} benchmarks='${BENCHMARKS}' n=${N_SAMPLES} temperature=${TEMPERATURE} top_p=${TOP_P} max_tokens=${MAX_TOKENS} seed=${SEED} enable_thinking=${ENABLE_THINKING} tp=${TENSOR_PARALLEL} official_parallel=${CODE_OFFICIAL_EVAL_PARALLEL} bigcodebench_parallel=${BIGCODEBENCH_OFFICIAL_EVAL_PARALLEL} skip_completed=${SKIP_COMPLETED} lcb_release=${LCB_RELEASE_VERSION} output_root=${OUTPUT_ROOT} status=${STATUS_FILE}"
    validate_inputs
    if [ "${DRY_RUN}" != "1" ] && [ "${ALLOW_DEEPCODER_INSTRUCT2507_R8K_OFFLINE_EVAL:-0}" != "1" ]; then
        log "ERROR: non-dry-run requires ALLOW_DEEPCODER_INSTRUCT2507_R8K_OFFLINE_EVAL=1"
        exit 1
    fi
    ...
    if [ "${DRY_RUN}" != "1" ]; then
        python3 "${SCRIPT_DIR}/summarize_code_offline_eval_n3.py" \
            ...
            --labels "${LABELS[@]}" \
            --summary-json "${OUTPUT_ROOT}/summary_deepcoder_instruct2507_r8k_unified_n3.json" \
            --summary-md "${OUTPUT_ROOT}/summary_deepcoder_instruct2507_r8k_unified_n3.md" \
            --title "DeepCoder Instruct-2507 R8K Unified N=3 Offline Eval" \
            --setting-description "Generation setting: n=3, temperature=1.0, top_p=0.95, max_tokens=4096, seed=42, enable_thinking=true, LiveCodeBench release_v5."
    fi
    if [ "${DRY_RUN}" = "1" ]; then
        log "DeepCoder Instruct2507 R8K unified N=3 offline eval queue dry-run complete. Planned results root: ${OUTPUT_ROOT}"
    else
        log "DeepCoder Instruct2507 R8K unified N=3 offline eval queue complete. Results root: ${OUTPUT_ROOT}"
    fi
    if [ "${DRY_RUN}" != "1" ]; then
        notify "DeepCoder Instruct R8K offline eval complete" "Status: completed\nWhat happened: unified N=3 official code eval queue finished.\nEvidence: ${OUTPUT_ROOT}/summary_deepcoder_instruct2507_r8k_unified_n3.json\nNext action: compare against KodCode v2_latest_unified_n3."
    fi
}
