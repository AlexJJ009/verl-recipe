#!/usr/bin/env bash
# Internal Stage123 queue implementation. The public entrypoint delegates lifecycle
# ownership to scripts/experiment_execution_core.py.
set -euo pipefail
...
PY
    ...
    stage2_ckpt=$(launch_and_wait "$stage2_tmux" "$stage2_prefix" "$STAGE2_STEPS" \
        ...
        STAGE123_RUN_ID="$stage2_id" STAGE123_MANIFEST="$STAGE123_MANIFEST" STAGE123_NORMALIZED_MANIFEST="$STAGE123_NORMALIZED_MANIFEST" STAGE123_ADMISSION_BUNDLE="$STAGE123_ADMISSION_BUNDLE" STAGE123_PREFLIGHT_POLICY="$STAGE123_PREFLIGHT_POLICY" STAGE123_PREFLIGHT_RESULT_MAX_AGE_SECONDS="$STAGE123_PREFLIGHT_RESULT_MAX_AGE_SECONDS" STAGE123_EXPECTED_PROFILE_HASH="$profile_hash" STAGE123_FORMAL_QUEUE_ID="$STAGE123_FORMAL_QUEUE_ID")
