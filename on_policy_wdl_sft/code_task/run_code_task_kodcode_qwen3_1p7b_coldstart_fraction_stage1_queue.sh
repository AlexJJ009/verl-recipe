#!/usr/bin/env bash
# Host-side KodCode Qwen3-1.7B CTX8K Stage1 queue comparing code format
...
LOG_FILE=${LOG_FILE:-"${REPO_HOST}/recipe/on_policy_wdl_sft/code_task/run_code_task_kodcode_qwen3_1p7b_coldstart_fraction_stage1_queue.log"}
...
RUN_LABELS=(
  ...
  "kodcode-qwen3-1p7b-coldstart-frac25-ctx8k-s1-beta01"
  "kodcode-qwen3-1p7b-coldstart-frac50-ctx8k-s1-beta0"
  "kodcode-qwen3-1p7b-coldstart-frac50-ctx8k-s1-beta01"
)
RUN_SCRIPTS=(
...
if [ "${DRY_RUN:-0}" != "1" ] && [ "${ALLOW_KODCODE_QWEN3_1P7B_COLDSTART_FRACTION_STAGE1_TRAINING:-0}" != "1" ]; then
    echo "[kodcode qwen3 1p7b coldstart fraction stage1 queue] ERROR: non-dry-run requires ALLOW_KODCODE_QWEN3_1P7B_COLDSTART_FRACTION_STAGE1_TRAINING=1" >&2
    exit 1
fi
...
precheck_or_stop() {
    local init_model_path="$1" free_gb
    [ "${DRY_RUN:-0}" = "1" ] && return 0
    [ -d "$init_model_path" ] || { log "ERROR init model not found: $init_model_path"; exit 1; }
    ...
    if [ "$free_gb" -lt "$MIN_FREE_GB" ]; then
        ...
        notify "KodCode Qwen3 1.7B coldstart fraction Stage1 blocked" "Disk gate failed: free=${free_gb}G need>=${MIN_FREE_GB}G"
        exit 1
    fi
}
...
wait_for_tmux_run() {
    ...
    if [ -z "$ckpt" ] || [ "$step" -lt "$final_step" ] || [ -z "$metrics" ]; then
        ...
        notify "KodCode Qwen3 1.7B coldstart fraction Stage1 failed" "${label} stopped before final checkpoint/metrics."
    ...
    notify "KodCode Qwen3 1.7B coldstart fraction Stage1 complete" "${label} reached step ${step}."
}

launch_container() {
    local container_script="$1" tmux_name="$2" prefix="$3" init_model_path="$4"
    local host_script="${container_script/#$REPO_CONTAINER/$REPO_HOST}"
    local extra_env
    extra_env="RUN_PREFIX='$prefix' INIT_MODEL_PATH='$init_model_path' PROTECTED_CKPT_STEPS='$PROTECTED_CKPT_STEPS_DEFAULT'"
...
notify "KodCode Qwen3 1.7B coldstart fraction Stage1 queue started" "range=${START_INDEX}-${END_INDEX}; min_free_gb=${MIN_FREE_GB}"
...
for idx in "${!RUN_LABELS[@]}"; do
    ...
    init_model_path="${RUN_INIT_MODEL_PATHS[$idx]}"
    CURRENT_IDX="$idx"
    CURRENT_LABEL="$label"
    ...
    precheck_or_stop "$init_model_path"
    launch_container "${RUN_SCRIPTS[$idx]}" "$tmux_name" "$prefix" "$init_model_path"
    wait_for_tmux_run "$tmux_name" "$prefix" "$final_step" "$idx" "$label" || continue
done

log "kodcode qwen3 1p7b coldstart fraction stage1 queue complete"
notify "KodCode Qwen3 1.7B coldstart fraction Stage1 queue complete" "status_file=${QUEUE_STATUS_FILE}"
