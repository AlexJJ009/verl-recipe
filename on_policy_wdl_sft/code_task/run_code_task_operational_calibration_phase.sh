...
common=(
  ...
  CODE_VAL_FILES="['$CALIBRATION_VAL_FILE']" TEST_FILES="['$CALIBRATION_VAL_FILE']" VAL_MAX_SAMPLES=64
  ...
  CODE_REWARD_NUM_WORKERS=1 CODE_REWARD_TIMEOUT=30 CODE_REWARD_MANAGER_TIMEOUT=30
)
case "$PHASE" in
 stage1)
  env "${common[@]}" INIT_MODEL_PATH="${QWEN3_1P7B_MODEL_PATH:?}" WDL_SFT_BETA=0.1 bash "$SCRIPT_DIR/run_s1_code_base.sh"
  ;;
 stage2)
  : "${CALIBRATION_STAGE1_CKPT_DIR:?}" "${CALIBRATION_STAGE1_MODEL2:?}"
  env "${common[@]}" BASE_MODEL_PATH="${QWEN3_1P7B_MODEL_PATH:?}" SUBMODEL_KL_MODEL1_REF_PATH="$QWEN3_1P7B_MODEL_PATH" MODEL_PATH="$CALIBRATION_OUTPUT_ROOT/joint_model" MODEL2_CACHE_TAG=calibration-s2-1p7b STAGE1_CKPT_DIR="$CALIBRATION_STAGE1_CKPT_DIR" STAGE1_RUN_PREFIX="${CALIBRATION_STAGE1_RUN_PREFIX:?}" STAGE2_HANDOFF_STEP="${CALIBRATION_STAGE1_HANDOFF_STEP:?}" MERGED_MODEL2_DIR="$CALIBRATION_STAGE1_MODEL2" MODEL2_PATH="$CALIBRATION_STAGE1_MODEL2" REQUIRE_MERGED_MODEL2_PROVENANCE=False CODE_TRAIN_FILE="${CALIBRATION_TRAIN_FILE:?}" TRAIN_FILE="$CALIBRATION_TRAIN_FILE" WDL_SFT_BETA=0.1 EXPECTED_STAGE1_BETA=0.1 FUSION_LAMBDA=0.8 bash "$SCRIPT_DIR/run_s2_code_model2_rollout_common.sh"
  ;;
 stage3)
  env "${common[@]}" INIT_MODEL_PATH="${CALIBRATION_STAGE3_MODEL_PATH:?}" WDL_SFT_BETA=0.1 DATA_SHUFFLE=False bash "$SCRIPT_DIR/run_s1_code_base.sh"
  ;;
