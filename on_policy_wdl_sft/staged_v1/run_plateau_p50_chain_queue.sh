#!/usr/bin/env bash
# Sequential plateau handoff queue: Stage1 step 50 -> fixed merge -> Stage2 40 steps.
...
LOG_FILE=${LOG_FILE:-"${REPO_HOST}/recipe/on_policy_wdl_sft/staged_v1/run_plateau_p50_chain_queue.log"}
...
STAGE1_TOTAL_TRAINING_STEPS=${STAGE1_TOTAL_TRAINING_STEPS:-50}
STAGE1_HANDOFF_STEP=${STAGE1_HANDOFF_STEP:-50}
...
STAGE1_SCRIPTS=(
    "${REPO_CONTAINER}/recipe/on_policy_wdl_sft/staged_v1/run_s1_plateau_p50_beta_0.sh"
    "${REPO_CONTAINER}/recipe/on_policy_wdl_sft/staged_v1/run_s1_plateau_p50_beta_01.sh"
)
STAGE1_TMUX_NAMES=(
...
STAGE2_SCRIPTS=(
    "${REPO_CONTAINER}/recipe/on_policy_wdl_sft/staged_v1/run_s2_plateau_p50_beta0_beta0.sh"
    "${REPO_CONTAINER}/recipe/on_policy_wdl_sft/staged_v1/run_s2_plateau_p50_beta01_beta01.sh"
)
STAGE2_TMUX_NAMES=(
...
has_conflicting_training() {
    ...
    session_count=$(tmux list-sessions -F '#S' 2>/dev/null \
        | grep -Ev "^${QUEUE_TMUX}$" \
        | grep -Ec '(^staged_v1_(chain|p50|p60)_s[12]_|^staged_v1_s[12]_|^ablation_|^wdl_|^train_)' || true)
...
assert_no_unapproved_collision() {
    ...
    notify "Plateau P50 WDL-SFT chain blocked" "Status: blocked
...
launch_run() {
    ...
    tmux has-session -t "$tmux_name" 2>/dev/null || {
        ...
        notify "Plateau P50 WDL-SFT chain launch failed" "Status: failed
...
wait_for_checkpoint_completion() {
    ...
    while true; do
        ...
        if [ "$tmux_state" = "missing" ]; then
            ...
            notify "Plateau P50 WDL-SFT chain run failed" "Status: failed
...
wait_for_stage2_completion() {
    ...
    while true; do
        ...
        if [ "$tmux_state" = "missing" ]; then
            ...
            notify "Plateau P50 WDL-SFT Stage2 failed" "Status: failed
...
merge_stage1_model2() {
    ...
    notify "Plateau P50 WDL-SFT merge starting" "Status: started
...
Next action: Stage2 will start after load check passes."
    ...
    if ! has_merged_model2 "$merged_dir"; then
        ...
        notify "Plateau P50 WDL-SFT merge failed" "Status: failed
...
Next action: Inspect the merge log."
    ...
    notify "Plateau P50 WDL-SFT merge complete" "Status: completed
...
log "plateau P50 Stage1->Stage2 chain queue started"
notify "Plateau P50 WDL-SFT chain started" "Status: started
What happened: Starting sequential plateau handoff chains: P50-B0 then P50-B01.
...
for idx in "${!BETA_LABELS[@]}"; do
    ...
    notify "Plateau P50 WDL-SFT chain branch started" "Status: started
...
Next action: Waiting for resources."
    ...
    if ! is_complete "$s1_prefix" "$STAGE1_TOTAL_TRAINING_STEPS"; then
        ...
        notify "Plateau P50 Stage1 launched" "Status: started
...
Next action: Waiting for Stage1 completion."
    ...
    notify "Plateau P50 Stage1 complete" "Status: completed
...
Next action: Fixed-step Model2 merge."
    ...
    notify "Plateau P50 Stage2 launched" "Status: started
...
Next action: Waiting for final checkpoint and metrics."
    ...
    notify "Plateau P50 Stage2 complete" "Status: completed
...
log "plateau P50 Stage1->Stage2 chain queue complete"
notify "Plateau P50 WDL-SFT chain complete" "Status: completed
What happened: P50 plateau handoff optional ablation completed.
