...
> **Post-fix rerun status (2026-05-03, L40S migration audit)**: the first label-fix rerun, `WDL-SFT-Qwen3-4B-MATH-2A-BASE-LABELFIX_1777346990` from `run_2a_base.sh`, finished on the retired A800 before the 2026-04-30 shutdown. On the current L40S host, the checkpoint run dir is present at `/data-1/checkpoints/WDL-SFT-Qwen3-4B-MATH-2A-BASE-LABELFIX_1777346990/`; `latest_checkpointed_iteration.txt=300`, and the promoted path is `/data-1/checkpoints/WDL-SFT-Qwen3-4B-MATH-2A-BASE-LABELFIX_1777346990/global_step_300`. However, the canonical training log, validation jsonls, and `/data-1/model_weights/` extraction dir for this run were **not found** during the audit; only `/data-1/wandb_runs/WDL-SFT-Qwen3-4B-MATH-2A-BASE-LABELFIX/` remains as side evidence. The step-300 checkpoint also looks incomplete relative to earlier saves (missing `data.pt`, `fsdp_config.json`, tokenizer files, and most optimizer shards), so offline eval / merge / extract should stay blocked until loadability is verified or missing files are restored. All other affected `wdl_sft_is` reruns (`1a`, `1b`, `1c`, `2a-sft`, `2b-base`, `2b-sft`, `2c-base`, `2c-sft`) have **not** been rerun under their `-LABELFIX` prefixes yet.
---
