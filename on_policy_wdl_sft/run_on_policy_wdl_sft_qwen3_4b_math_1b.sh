#!/usr/bin/env bash
# ==============================================================================
# Experiment 1b: WDL-SFT-IS (v2) — Bidirectional + IS/clip — lr=5e-7, β=0.1
# Model: Qwen3-4B Joint Model (Weak=Base + Strong=SFT-stage-1)
# Algorithm: WDL-SFT v2 with reverse SFT re-test (β=0.1)
#
# Based on: run_on_policy_wdl_sft_qwen3_4b_math_1a.sh
# Diff from 1a:
#   - WDL_SFT_BETA: 0.0 → 0.1  (enables reverse SFT L- term)
#   - RUN_PREFIX only (everything else identical)
#
# Purpose: under v2 loss (ratio binary-mask clip on both positive and negative
# branches), re-test whether reverse SFT is actually unstable. Previous
# judgment (EXP-14 M5.6 model1 −21.6%) was made under v1 loss with no clip
# protection — the collapse may have been a side-effect of missing stability
# mechanisms, not of β>0 per se.
#
# Decision criterion:
#   - 1b model2 ≥ 1a model2 → reverse SFT has incremental value; update CLAUDE.md
#   - 1b model2 < 1a model2 → reverse SFT genuinely doesn't help
#
# Plan reference: docs/joint_training/plans/active/wdl_sft_is.md (§4 row 1b)
#
# Hardware target: 8× A800-SXM4-80GB (640 GB total)
#
# Usage:
#   bash recipe/on_policy_wdl_sft/run_on_policy_wdl_sft_qwen3_4b_math_1b.sh
# ==============================================================================
...
RUN_PREFIX=${RUN_PREFIX:-"WDL-SFT-Qwen3-4B-MATH-1B-LABELFIX"}
...
# β=0.1: bidirectional SFT (reverse term re-enabled under v2 IS/clip protection)
...
if [ "${USE_REMOVE_PADDING}" = "True" ]; then
    if ! python3 -c "import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('flash_attn') else 1)" \
        >/dev/null 2>&1; then
        echo "[wdl-sft-is-1b] WARNING: flash_attn is not installed; disabling USE_REMOVE_PADDING." >&2
...
# 1b: Bidirectional run with v2 loss (β=0.1, 300 steps). Validate & save every 25 steps.
