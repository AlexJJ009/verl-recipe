#!/usr/bin/env bash
# ==============================================================================
# Experiment 1c: WDL-SFT-IS (v2) — Forward-Only + IS/clip — lr=1e-6, β=0
# Model: Qwen3-4B Joint Model (Weak=Base + Strong=SFT-stage-1)
# Algorithm: WDL-SFT v2 at higher lr (matches EXP-15 LR3 rate)
#
# Based on: run_on_policy_wdl_sft_qwen3_4b_math_1a.sh
# Diff from 1a:
#   - LR: 5e-7 → 1e-6 (doubled, same as EXP-15 LR3)
#   - RUN_PREFIX only (everything else identical)
#
# Purpose: under v2 loss (IS/clip protection), re-test whether lr=1e-6 is
# actually too high. EXP-15 LR3 (v1 loss at lr=1e-6) peaked at step 125 then
# drifted — was the drift caused by the lr itself, or by v1's missing
# stability mechanisms? This experiment disambiguates.
#
# Decision criterion:
#   - 1c trains stably through 300 steps AND reaches/exceeds 1a's peak → v1
#     drift was a loss-layer issue, not an lr issue; safe to use higher lr
#   - 1c still drifts post-~step 125 → lr=1e-6 is genuinely past the stable
#     region; stick with 5e-7 as default
#
# Plan reference: docs/joint_training/plans/active/wdl_sft_is.md (§4 row 1c)
#
# Hardware target: 8× A800-SXM4-80GB (640 GB total)
#
# Usage:
#   bash recipe/on_policy_wdl_sft/run_on_policy_wdl_sft_qwen3_4b_math_1c.sh
# ==============================================================================
...
if [ "${USE_REMOVE_PADDING}" = "True" ]; then
    if ! python3 -c "import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('flash_attn') else 1)" \
        >/dev/null 2>&1; then
        echo "[wdl-sft-is-1c] WARNING: flash_attn is not installed; disabling USE_REMOVE_PADDING." >&2
...
# 1c: Forward-only run with v2 loss at higher lr (300 steps). Validate every 25 steps.
