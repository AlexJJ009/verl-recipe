#!/usr/bin/env bash
# Offline eval: WDL-SFT 1C (EXP-18) step 150 — model2 (strong/trainable, index=1)
# n=3, 7 benchmarks with system prompt
#
# Pipeline:
#   1. Merge FSDP shards -> merged joint model (HF safetensors)
#   2. Extract model2 from joint weights
#   3. Run vLLM offline inference on model2
#
# Sibling script run_eval_1c_step150_model1.sh handles model1 (reuses the merged joint).
set -xeuo pipefail

FSDP_ACTOR_DIR=/data-1/checkpoints/WDL-SFT-Qwen3-4B-MATH-1C_1776768784/global_step_150/actor
MERGED_JOINT_PATH=/data-1/model_weights/WDL-SFT-4B-MATH-1C/step_150
MODEL2_PATH=/data-1/model_weights/WDL-SFT-4B-MATH-1C/step_150_model2
OUTPUT_DIR=/data-1/model_weights/WDL-SFT-4B-MATH-1C/step_150_model2/inference_n3
