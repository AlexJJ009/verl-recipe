#!/usr/bin/env bash
# ==============================================================================
# Milestone 5.5 (M5.5): Forward-Only On-Policy WDL-SFT Training on MATH
...
#
# Based on: run_on_policy_wdl_sft_qwen3_4b_math.sh (M5 script)
# Key changes from M5:
#   - β=0.0 (reverse SFT disabled — forward-only on correct rollouts)
#   - lr=5e-7 (halved from M5's 1e-6 to reduce clipping pressure)
#   - total_training_steps=300 (shortened from 1745)
#   - save_freq=25, test_freq=25 (checkpoint + validation every 25 steps)
#   - max_actor_ckpt_to_keep=13 (retain all checkpoints, no silent pruning)
#   - calculate_entropy=True (diagnostic entropy logging, no regularization)
#   - All other hyperparameters identical to M5
#
...
RUN_PREFIX=${RUN_PREFIX:-"WDL-SFT-Qwen3-4B-MATH-M5-5"}
...
# M5.5: Short forward-only run (300 steps). Validate & save every 25 steps.
