#!/usr/bin/env bash
# ==============================================================================
# LR-3: Forward-Only On-Policy WDL-SFT — Learning Rate Search (lr=1e-6)
# Model: Qwen3-4B Joint Model (Weak=Base + Strong=SFT-stage-1, ~8B total)
# Algorithm: Forward-only WDL-SFT (β=0, SFT on correct rollouts only)
#
# Based on: run_on_policy_wdl_sft_qwen3_4b_math_m5_5.sh (M5.5 baseline)
# Key changes from M5.5:
#   - lr=1e-6 (doubled from M5.5's 5e-7)
#   - RUN_PREFIX updated to LR-3
#   - All other hyperparameters identical to M5.5
#
# Part of LR search grid: see docs/joint_training/plans/active/lr_search.md
#
# Hardware target: 8× A800-SXM4-80GB (640 GB total)
#
# Usage:
#   bash recipe/on_policy_wdl_sft/run_on_policy_wdl_sft_qwen3_4b_math_lr3.sh
# ==============================================================================
...
RUN_PREFIX=${RUN_PREFIX:-"WDL-SFT-Qwen3-4B-MATH-LR3"}
...
# ===================== Section 8: WDL-SFT Algorithm Config ====================
...
# No KL penalty for WDL-SFT (pure SFT-style training)
...
# No IS correction for WDL-SFT (not needed for SFT-style loss)
rollout_is="null"
...
# 8192 (reduced from 9192) + gpu_util=0.35 (reduced from 0.4) gives ~4.9 GiB headroom for
# entropy logsumexp chunks (vocab=151936, joint model). See OOM postmortem at step 132.
actor_ppo_max_token_len=${ACTOR_PPO_MAX_TOKEN_LEN:-8192}
...
if [ "${USE_REMOVE_PADDING}" = "True" ]; then
    if ! python3 -c "import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('flash_attn') else 1)" \
        >/dev/null 2>&1; then
        echo "[wdl-sft-4b] WARNING: flash_attn is not installed; disabling USE_REMOVE_PADDING." >&2
...
# LR-3: Forward-only run (300 steps, lr=1e-6). Validate & save every 25 steps.
