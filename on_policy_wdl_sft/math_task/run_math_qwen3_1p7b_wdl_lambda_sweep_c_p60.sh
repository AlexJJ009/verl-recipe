...
if [ "$#" -ne 0 ]; then
    echo "ERROR: lambda-sweep C accepts no positional/Hydra overrides" >&2
    exit 64
fi
: "${FUSION_LAMBDA:?set FUSION_LAMBDA to 0.4 or 0.5}"

case "$FUSION_LAMBDA" in
    0.4) lambda_tag=lambda04 ;;
    0.5) lambda_tag=lambda05 ;;
    *)
        echo "ERROR: formal C lambda sweep permits only FUSION_LAMBDA=0.4 or 0.5" >&2
        exit 64
        ;;
esac
export FUSION_LAMBDA

# The only scientific treatment is FUSION_LAMBDA. Everything else is pinned to
# the admitted Math causal-P60 C contract and uses lambda-tagged identities.
export RUN_PREFIX="MATH-WDL-${lambda_tag^^}-ARM-C-P60-QWEN3-1P7B"
...
export MODEL_PATH="/data-1/.cache/huggingface/math-wdl-${lambda_tag}-arm-c-p60"
...
export WANDB_PROJECT=OnPolicyWDLSFT-Math-1P7B-Lambda-Sweep-C-P60
export CAUSAL_ARTIFACT_ROOT="/data-2/model_weights/math_task/qwen3_1p7b_wdl_lambda_sweep/${lambda_tag}/standard-c-p60"
