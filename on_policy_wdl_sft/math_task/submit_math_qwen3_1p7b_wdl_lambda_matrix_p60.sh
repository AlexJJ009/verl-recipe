...
ARM_VALUES=(fixed-m1 d0 standard-c fixed-m1 d0 standard-c)
LAMBDA_VALUES=(0.5 0.5 0.7 0.4 0.4 0.9)
NICE_VALUES=(0 10 20 100 110 120)
...
for i in "${!ARM_VALUES[@]}"; do echo "PLAN priority=$i arm=${ARM_VALUES[$i]} lambda=${LAMBDA_VALUES[$i]} P60"; done
...
expected = {"status":"authorized", "experiment_id":"math_qwen3_1p7b_wdl_lambda_matrix_p60",
...
expected_runs = {"lambda04-fixed", "lambda05-fixed", "lambda04-d0", "lambda05-d0", "lambda07-c", "lambda09-c"}
...
printf 'job_id\tarm\tfusion_lambda\tnice\tparent\trecipe\timage\n' >"$jobs_file"
for i in "${!ARM_VALUES[@]}"; do
    arm="${ARM_VALUES[$i]}"; lam="${LAMBDA_VALUES[$i]}"; nice="${NICE_VALUES[$i]}"
    name="LM-${arm}-l${lam/./}-P60"
    job="$(sbatch --parsable --hold --nodelist="$LAMBDA_MATRIX_ALLOWED_NODES" --nice="$nice" \
        --job-name="$name" \
        --export="ALL,LAMBDA_ARM=$arm,FUSION_LAMBDA=$lam,LAMBDA_MATRIX_PARENT_SHA=$PARENT_SHA,LAMBDA_MATRIX_RECIPE_SHA=$RECIPE_SHA,LAMBDA_MATRIX_IMAGE_ID=$IMAGE_ID,LAMBDA_MATRIX_EVIDENCE_RELAY_HOST=$LAMBDA_MATRIX_EVIDENCE_RELAY_HOST,LAMBDA_MATRIX_NODE_ROOT_MAP=$LAMBDA_MATRIX_NODE_ROOT_MAP,LAMBDA_MATRIX_STAGE_REL=$LAMBDA_MATRIX_STAGE_REL" \
        "$SCRIPT_DIR/slurm/run_math_qwen3_1p7b_wdl_lambda_matrix_p60.sbatch")"
    submitted+=("$job")
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$job" "$arm" "$lam" "$nice" "$PARENT_SHA" "$RECIPE_SHA" "$IMAGE_ID" >>"$jobs_file"
done
