...
for name in SLURM_JOB_ID SLURMD_NODENAME CUDA_VISIBLE_DEVICES LAMBDA_ARM \
    FUSION_LAMBDA LAMBDA_MATRIX_PARENT_SHA LAMBDA_MATRIX_RECIPE_SHA \
...
case "$LAMBDA_ARM:$FUSION_LAMBDA" in
    ...
    *) die "unauthorized arm/lambda pair" ;;
esac
run_prefix="MATH-WDL-${lambda_tag^^}-ARM-${arm_tag}-P60-QWEN3-1P7B"
family="$run_prefix"
...
python3 - "$launch_receipt" "$LAMBDA_ARM" "$FUSION_LAMBDA" \
    "$LAMBDA_MATRIX_PARENT_SHA" "$LAMBDA_MATRIX_RECIPE_SHA" "$LAMBDA_MATRIX_IMAGE_ID" <<'PY'
import json, sys
from pathlib import Path
path = Path(sys.argv[1])
arm, lam, parent, recipe, image = sys.argv[2:]
receipt = json.loads(path.read_text())
expected = {
    "status": "authorized",
    "experiment_id": "math_qwen3_1p7b_wdl_lambda_matrix_p60",
...
run_id = f"lambda{lam.replace('.', '')}-{'fixed' if arm == 'fixed-m1' else arm.replace('standard-', '')}"
authorized = {(run["id"], str(run["fusion_lambda"])) for run in receipt.get("runs", [])}
if (run_id, lam) not in authorized:
...
artifact_root="${data2_host}/model_weights/math_task/qwen3_1p7b_wdl_lambda_matrix/${lambda_tag}/${artifact_arm}-p60"
job_root="${artifact_root}/slurm/${SLURM_JOB_ID}"
test ! -e "$job_root" || die "job root already exists"
mkdir -p "$job_root"
relay_job_root="/data-1/code/_artifacts/verl-v0.7/math-lambda-matrix/${LAMBDA_MATRIX_PARENT_SHA}/${SLURM_JOB_ID}"
container_name="lambda-matrix-${SLURM_JOB_ID}-${artifact_arm}-${lambda_tag}"
training_pid="" gate_pid="" run_name="" completion_recorded=false

discover_run_name() {
    [ -n "$run_name" ] && return 0
    local metrics_dir="${artifact_root}/logs/metrics/OnPolicyWDLSFT-Math-1P7B-Lambda-Matrix-P60"
    local -a candidates=()
...
cleanup() {
    ...
    printf '{"job_id":"%s","node":"%s","arm":"%s","fusion_lambda":%s,"training_exit_code":%s,"completion_recorded":%s,"evidence_relayed":%s,"parent_sha":"%s","recipe_sha":"%s","image_id":"%s"}\n' \
        "$SLURM_JOB_ID" "$SLURMD_NODENAME" "$LAMBDA_ARM" "$FUSION_LAMBDA" "$terminal_rc" \
...
printf '{"job_id":"%s","node":"%s","arm":"%s","fusion_lambda":%s,"parent_sha":"%s","recipe_sha":"%s","image_id":"%s","job_root":"%s","relay_root":"%s"}\n' \
    "$SLURM_JOB_ID" "$SLURMD_NODENAME" "$LAMBDA_ARM" "$FUSION_LAMBDA" \
...
python3 "$repo_host/scripts/math_wdl_first_step_gate.py" \
    ...
    --project OnPolicyWDLSFT-Math-1P7B-Lambda-Matrix-P60 \
...
metrics_dir="${artifact_root}/logs/metrics/OnPolicyWDLSFT-Math-1P7B-Lambda-Matrix-P60"
