#!/usr/bin/env bash
# Format cold-start SFT on KodCode with Qwen3-1.7B.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"

export RUN_PREFIX=${RUN_PREFIX:-SFT-FORMAT-COLDSTART-Qwen3-1P7B-CODE-KODCODE-V1}
...
export TRAIN_FILE=${TRAIN_FILE:-/data-1/dataset/code/format_cold_start/kodcode_light_sft_messages.parquet}
...
export MAX_TOKEN_LEN_PER_GPU=${MAX_TOKEN_LEN_PER_GPU:-9216}
export LR=${LR:-5e-6}
...
export TRAIN_SEED=${TRAIN_SEED:-20260706}
...
if [ ! -d "$MODEL_PATH" ]; then
    echo "[format-sft-code] ERROR: MODEL_PATH not found: $MODEL_PATH" >&2
    exit 1
fi
if [ ! -f "$TRAIN_FILE" ]; then
    if [ "${DRY_RUN:-0}" = "1" ]; then
        echo "[format-sft-code] WARNING: TRAIN_FILE not found during DRY_RUN: $TRAIN_FILE" >&2
    else
        echo "[format-sft-code] ERROR: TRAIN_FILE not found: $TRAIN_FILE" >&2
        exit 1
    fi
fi
if [ "${DRY_RUN:-0}" != "1" ] && [ "${ALLOW_FORMAT_COLD_START_SFT:-0}" != "1" ]; then
    echo "[format-sft-code] ERROR: non-dry-run requires ALLOW_FORMAT_COLD_START_SFT=1" >&2
    exit 1
fi
...
printf '[format-sft-code] command:'
printf ' %q' "${CMD[@]}"
printf '\n'

if [ "${DRY_RUN:-0}" = "1" ]; then
    echo "[format-sft-code] DRY_RUN=1; exiting before training"
    exit 0
fi
