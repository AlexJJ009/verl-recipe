#!/usr/bin/env bash
# Shared launcher for WDL group-advantage IS joint-model runs.
set -xeuo pipefail
...
echo "LOSS_MODE  : ${LOSS_MODE:-wdl_group_adv_is}"
echo "LR         : $LR"
...
VALIDATION_OUTPUT_DIR=${VALIDATION_OUTPUT_DIR:-"${LOG_DIR}/validation/${WANDB_RUN_NAME}"}
...
CUSTOM_REWARD_FN_PATH=${CUSTOM_REWARD_FN_PATH:-"${REPO_ROOT}/recipe/on_policy_wdl_sft/custom_reward_function_latex_verify.py"}
...
[ -f "$CUSTOM_REWARD_FN_PATH" ] || { echo "ERROR: CUSTOM_REWARD_FN_PATH not found: $CUSTOM_REWARD_FN_PATH" >&2; exit 1; }
...
loss_mode=${LOSS_MODE:-wdl_group_adv_is}
...
if [ -n "${OVERLONG_BUFFER_LEN+x}" ]; then
    overlong_buffer_len=${OVERLONG_BUFFER_LEN}
    if [ "$overlong_buffer_len" -gt "$max_response_length" ]; then
        echo "ERROR: OVERLONG_BUFFER_LEN=${overlong_buffer_len} exceeds MAX_RESPONSE_LENGTH=${max_response_length}." >&2
        exit 1
    fi
else
...
if [ "${USE_REMOVE_PADDING}" = "True" ]; then
    if ! python3 -c "import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('flash_attn') else 1)" >/dev/null 2>&1; then
        echo "[group-adv-is] WARNING: flash_attn is not installed; disabling USE_REMOVE_PADDING." >&2
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.policy_loss.pos_sft_fallback_coef=${POS_SFT_FALLBACK_COEF} \
